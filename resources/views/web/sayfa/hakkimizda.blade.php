<x-layout.web>
    ...
    @php
        $sayfa1 = $sayfaYonetim->where('id', 1)->first();
        $dilVerisi1 = $sayfa1 ? $sayfa1->sayfaYonetimDiller->first() : null;
    @endphp

    @foreach ($sayfa_yonetim as $sayfa_yonet)
        @if ($sayfa_yonet->id == 1)
            @php
                $sayfa_yonetimDilVerisi = $sayfa_yonet->sayfa_yonetimDiller
                    ? $sayfa_yonet->sayfa_yonetimDiller->where('dil', $aktifDil)->first()
                    : null;
            @endphp
            <!-- About Section -->
                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-6 mb-5 mb-lg-0">
                            <div class="position-relative">
                                <img src="{{ Storage::url($sayfa_yonet->resim_url) }}"
                                    class="img-fluid rounded-4 shadow-lg" alt="About Us">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="ps-lg-5">
                                <span class="badge bg-success bg-opacity-10 text-white m-3 badge-success-size">
                                    <i class="fas fa-leaf me-2"></i> Hakkımızda
                                </span>
                                <h2 class="display-6 fw-bold mb-4">{{ $dilVerisi1->baslik }}
                                </h2>
                                <div class="mb-4">
                                    {!! $dilVerisi1->icerik !!}
                                </div>
