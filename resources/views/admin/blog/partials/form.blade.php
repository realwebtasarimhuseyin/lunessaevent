<form id="blogForm" onsubmit="(return false)" class="mb-4 layout-spacing layout-top-spacing">
    <div class="row w-100">
        <div class="col-12">
            <div class="widget-content widget-content-area form-section">
                <label for="blogResim">Görsel</label>
                <div class="multiple-file-upload lvvl">
                    <input type="file" class="file-upload-multiple filepond" id="blogResim" name="blogResim" />
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="tab-pane fade show active" id="pills-genel" role="tabpanel"
                    ...
                    <div class="simple-pill">
                        ...
                        <div class="tab-content" id="pills-tabContent">
                            @foreach ($desteklenenDil as $dil)
                                @php
                                    $dilVerisi = isset($blog) ? $blog->blogDiller->where('dil', $dil)->first() : null;
                                @endphp
                ...
                <div class="col-xxl-12 mb-4 lvvl">
                    <div class="switch form-switch-custom switch-inline form-switch-primary">
                        <input class="switch-input input" type="checkbox" role="switch" id="durum"
                            {{ !isset($blog) || (isset($blog) && $blog->durum) ? 'checked' : '' }}>
                        <label class="switch-label" for="durum">Durum</label>
                    </div>
                </div>

                <div class="col-xxl-12 col-sm-4 col-12 mx-auto" id="blogBtnCnt">
                    <button class="btn btn-success w-100" id="blogBtnSbt" type="button">
                        {{ $btnText }}
                    </button>
                </div>
            </div>
...
@isset($blog)
    <script>
        ...
        const blogIcerik = {};
        @if ($blog->resim_url)
            const resimUrl = "{{ Storage::url($blog->resim_url) }}";
        @endif

        @foreach ($desteklenenDil as $dil)
            @php
                $dilVerisi = $blog->blogDiller->where('dil', $dil)->first();
            @endphp

            @if ($dilVerisi)
                blogIcerik['{{ $dil }}'] = {!! json_encode($dilVerisi->icerik) !!};
            @else
                blogIcerik['{{ $dil }}'] = "";
            @endif
        @endforeach
