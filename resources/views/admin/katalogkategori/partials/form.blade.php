<form id="katalogKategoriForm" onsubmit="(return false)" class="mb-4 layout-spacing layout-top-spacing">
        ...
        <div class="col-12">
            <div class="simple-pill">
                ...
                <div class="tab-content" id="pills-tabContent">
                    @foreach ($desteklenenDil as $dil)
                        @php
                            $dilVerisi = isset($katalogKategori)
                                ? $katalogKategori->katalogKategoriDiller->where('dil', $dil)->first()
                                : null;
                        @endphp
                    ...
                    <div class="col-auto mb-4 lvvl">
                        <div class="switch form-switch-custom switch-inline form-switch-primary">
                            <input class="switch-input input" type="checkbox" role="switch" id="durum"
                                {{ !isset($katalogKategori) || (isset($katalogKategori) && $katalogKategori->durum) ? 'checked' : '' }}>
                            <label class="switch-label" for="durum">Kategori Durum</label>
                        </div>
                    </div>
                    <div class="col-xxl-12 col-sm-4 col-12 mx-auto" id="katalogKategoriBtnCnt">
                        <button class="btn btn-success w-100" id="katalogKategoriBtnSbt" type="button">
                            {{ $btnText }}
                        </button>
                    </div>
                </div>
...
@isset($katalogKategori)
    <script>
        const katalogKategoriId = {{ $katalogKategori->id }};

        @if ($katalogKategori->resim_url)
            const katalogKategoriResimUrl = "{{ Storage::url($katalogKategori->resim_url) }}";
        @endif
    </script>
@endisset
