<form id="hizmetKategoriForm" onsubmit="(return false)" class="mb-4 layout-spacing layout-top-spacing">
        ...
        <div class="col-12">
            <div class="simple-pill">
                ...
                <div class="tab-content" id="pills-tabContent">
                    @foreach ($desteklenenDil as $dil)
                        @php
                            $dilVerisi = isset($hizmetKategori)
                                ? $hizmetKategori->hizmetKategoriDiller->where('dil', $dil)->first()
                                : null;
                        @endphp
                    ...
                    <div class="col-auto mb-4 lvvl">
                        <div class="switch form-switch-custom switch-inline form-switch-primary">
                            <input class="switch-input input" type="checkbox" role="switch" id="durum"
                                {{ !isset($hizmetKategori) || (isset($hizmetKategori) && $hizmetKategori->durum) ? 'checked' : '' }}>
                            <label class="switch-label" for="durum">Kategori Durum</label>
                        </div>
                    </div>
                    <div class="col-xxl-12 col-sm-4 col-12 mx-auto" id="hizmetKategoriBtnCnt">
                        <button class="btn btn-success w-100" id="hizmetKategoriBtnSbt" type="button">
                            {{ $btnText }}
                        </button>
                    </div>
                </div>
...
@isset($hizmetKategori)
    <script>
        const hizmetKategoriId = {{ $hizmetKategori->id }};
        const hizmetKategoriResimUrl = "{{ Storage::url($hizmetKategori->resim_url) }}";
    </script>
@endisset