<form id="sssForm" onsubmit="(return false)" class="mb-4 layout-spacing layout-top-spacing">
    <div class="row w-100">
        <div class="col-12">
            ...
            <div class="simple-pill">
                ...
                <div class="tab-content" id="pills-tabContent">
                    @foreach ($desteklenenDil as $dil)
                        @php
                            $dilVerisi = isset($sss) ? $sss->sssDiller->where('dil', $dil)->first() : null;
                        @endphp
                            ...
                            <div class="widget-content widget-content-area form-section">
                                ...
                                <div class="row mb-4">
                                    <div class="col-sm-12 lvvl">
                                        ...
                                        <div id="sssIcerik-{{ $dil }}" name="icerik-{{ $dil }}"
                                            class="input"></div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="widget-content widget-content-area form-section mt-3">
                <div class="col-xxl-12 mb-4 lvvl">
                    <div class="switch form-switch-custom switch-inline form-switch-primary">
                        <input class="switch-input input" type="checkbox" role="switch" id="durum"
                            {{ !isset($sss) || (isset($sss) && $sss->durum) ? 'checked' : '' }}>
                        <label class="switch-label" for="durum">Durum</label>
                    </div>
                </div>

                <div class="col-xxl-12 col-sm-4 col-12 mx-auto" id="sssBtnCnt">
                    <button class="btn btn-success w-100" id="sssBtnSbt" type="button">
                        {{ $btnText }}
                    </button>
                </div>
            </div>
...
@isset($sss)
    <script>
        ...
        @foreach ($desteklenenDil as $dil)
            @php
                $dilVerisi = $sss->sssDiller->where('dil', $dil)->first();
            @endphp

            @if ($dilVerisi)
                sssIcerik['{{ $dil }}'] = {!! json_encode($dilVerisi->icerik) !!};
            @else
                sssIcerik['{{ $dil }}'] = "";
            @endif
        @endforeach
