<form id="sektorlerForm" onsubmit="(return false)" class="row layout-spacing layout-top-spacing">
    <div class="row w-100">
        <div class="col-12">
            <div class="widget-content widget-content-area form-section mb-4">
                <div class="row">
                    <div class="col-12">
                        <label for="sektorlerResim">Sektör Resim</label>
                        <div class="multiple-file-upload lvvl">
                            <input type="file" class="sektorlerResim filepond" id="sektorlerResim" name="filepond" />
                        </div>
                    </div>
            ...
            <div class="simple-pill">
                ...
                <div class="tab-content" id="pills-tabContent">
                    ...
                    @foreach ($desteklenenDil as $dil)
                        @php
                            $dilVerisi = isset($sektor) ? $sektor->sektorlerDiller->where('dil', $dil)->first() : null;
                        @endphp
                            ...
                            <div class="widget-content widget-content-area form-section  mb-4">
                                <div class="row mb-4">
                                    <div class="col-xxl-12 lvvl">
                                        <label for="baslik">Sektör Başlık</label>
                                        <input type="text" class="form-control input"
                                            name="baslik-{{ $dil }}" id="baslik-{{ $dil }}"
                                            placeholder="sektorler Başlık" value="{{ $dilVerisi->baslik ?? '' }}">
                                    </div>
                                </div>

                                <div class="row mb-4">
                                    <div class="col-xxl-12 lvvl">
                                        <label for="kisaIcerik-{{ $dil }}">Sektör Kısa İçerik</label>
                                        <textarea type="text" class="form-control input" name="kisaIcerik-{{ $dil }}"
                                            id="kisaIcerik-{{ $dil }}" placeholder="Sektör Kısa İçerik">{{ $dilVerisi->kisa_icerik ?? '' }}</textarea>
                                    </div>
                                </div>

                                <div class="row mb-4">
                                    <div class="col-xxl-12 lvvl">
                                        <label for="icerik">Sektör İçerik</label>
                ...
                <div class="row">
                    <div class="col-xxl-12 mb-4 lvvl">
                        <div class="switch form-switch-custom switch-inline form-switch-primary">
                            ...
                            <label class="switch-label" for="durum">Sektör Durum</label>
                        </div>
                    </div>
                    <div class="col-xxl-12 col-sm-4 col-12 mx-auto" id="sektorlerBtnCnt">
                        <button class="btn btn-success w-100" id="sektorlerBtnSbt" type="button">
                            {{ $btnText }}
                        </button>
                    </div>
                </div>
...
@isset($sektor)
    <script>
        const sektorlerId = {{ $sektor->id }};
        const sektorlerIcerik = {};

        const sektorlerResimUrl = "{{ Storage::url($sektor->resim_url) }}";

        @foreach ($desteklenenDil as $dil)
            @php
                $dilVerisi = $sektor->sektorlerDiller->where('dil', $dil)->first();
            @endphp

            @if ($dilVerisi)
                sektorlerIcerik['{{ $dil }}'] = {!! json_encode($dilVerisi->icerik) !!};
            @else
                sektorlerIcerik['{{ $dil }}'] = "";
            @endif
        @endforeach
