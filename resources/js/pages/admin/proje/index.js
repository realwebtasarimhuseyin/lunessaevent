...
var RPProjeIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let projeDataTable;

    let ProjeTable = function () {
        projeDataTable = AjaxDataTables({
            id: "projeDataTable",
            ajax: {
                url: `${panelUrl}/proje/liste`,
                data: function (data) {
    ...
    const ProjeTableKontrol = function () {
        document
            .querySelector("#projeDataTable")
        ...
        projeDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => projeDataTable.row(row.node).data().sira_no)
                .sort((a, b) => a - b);

            const projeler = details.map((row, index) => {
                return {
                    id: projeDataTable.row(row.node).data().id,
            ...
            if (projeler.length > 0) {
                ...
                axios.post(`${panelUrl}/proje/siralamaduzenle`, { projeler: projeler })
    ...
    const handleDelete = (e) => {
        ...
        const data = projeDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Proje Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/proje/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
