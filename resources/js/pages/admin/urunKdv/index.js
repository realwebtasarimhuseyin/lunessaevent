...
const RPUrunKdvIndex = (function () {

    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let urunKdvDataTable;

    const UrunKdvTable = function () {
        urunKdvDataTable = AjaxDataTables({
            id: "urunKdvDataTable",
            ajax: {
                url: `${panelUrl}/urunkdv/liste`,
                data: function (data) {
                    data.ara = "";
                },
            },
            ...
            buttons: {
                print: {
                    dosyaAdi: "urunkdv-liste",
                    baslik: "Ürün Kdv Liste",
                    tabloSutun: [1, 2, 3, 4],
                },
        ...
        multiCheck(urunKdvDataTable);
    };
    const UrunKdvTableKontrol = function () {
        document
            .querySelector("#urunKdvDataTable")
    ...
    const handleDelete = (e) => {
        ...
        const data = urunKdvDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Ürün Kdv Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/urunkdv/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
    ...
    return {
        init: function () {
            UrunKdvTable(); UrunKdvTableKontrol();
        },
    };
})();
