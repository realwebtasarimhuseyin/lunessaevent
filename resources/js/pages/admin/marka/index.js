...
const RPMarkaIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let markaDataTable;

    const MarkaTable = function () {
        markaDataTable = AjaxDataTables({
            id: "markaDataTable",
            ajax: {
                url: `${panelUrl}/marka/liste`,
                data: function (data) {
                    data.ara = "";
                },
            },
    ...
    const MarkaTableKontrol = function () {
        document
            .querySelector("#markaDataTable")
        ...
        markaDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => markaDataTable.row(row.node).data().sira_no)
                .sort((a, b) => a - b);

            const markalar = details.map((row, index) => {
                return {
                    id: markaDataTable.row(row.node).data().id,
            ...
            if (markalar.length > 0) {
                ...
                axios.post(`${panelUrl}/marka/siralamaduzenle`, { markalar: markalar })
    ...
    const handleDelete = (e) => {
        ...
        const data = markaDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Marka Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/marka/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
