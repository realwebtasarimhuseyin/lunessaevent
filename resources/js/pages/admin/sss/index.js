...
const RPSSSIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let sssDataTable;

    const SSSTable = function () {
        sssDataTable = AjaxDataTables({
            id: "sssDataTable",
            ajax: {
                url: `${panelUrl}/sss/liste`,
                data: function (data) {
    ...
    const SSSTableKontrol = function () {
        document
            .querySelector("#sssDataTable")
        ...
        sssDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => sssDataTable.row(row.node).data().sira_no)
                .sort((a, b) => a - b);

            const sssler = details.map((row, index) => {
                return {
                    id: sssDataTable.row(row.node).data().id,
            ...
            if (sssler.length > 0) {
                ...
                axios.post(`${panelUrl}/sss/siralamaduzenle`, { sssler: sssler })
    ...
    const handleDelete = (e) => {
        ...
        const data = sssDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "SSS Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/sss/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
