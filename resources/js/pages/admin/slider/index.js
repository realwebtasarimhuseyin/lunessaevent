...
const RPSliderIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let sliderDataTable;

    const SliderTable = function () {
        sliderDataTable = AjaxDataTables({
            id: "sliderDataTable",
            ajax: {
                url: `${panelUrl}/slider/liste`,
                data: function (data) {
                    data.ara = "";
                },
            },
    ...
    const SliderTableKontrol = function () {
        document
            .querySelector("#sliderDataTable")
        ...
        sliderDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => sliderDataTable.row(row.node).data().sira_no)
                .sort((a, b) => a - b);

            const sliderlar = details.map((row, index) => {
                return {
                    id: sliderDataTable.row(row.node).data().id,
            ...
            if (sliderlar.length > 0) {
                ...
                axios.post(`${panelUrl}/slider/siralamaduzenle`, { sliderlar: sliderlar })
    ...
    const handleDelete = (e) => {
        ...
        const data = sliderDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Slider Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/slider/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
