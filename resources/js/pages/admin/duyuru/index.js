...
const RPDuyuruIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let duyuruDataTable;

    const DuyuruTable = function () {
        duyuruDataTable = AjaxDataTables({
            id: "duyuruDataTable",
            ajax: {
                url: `${panelUrl}/duyuru/liste`,
                data: function (data) {
                    data.ara = "";
                },
            },
            ...
            sutunTanim: [
...

                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        let trimmedText = text.length > 50 ? `${text.substring(0, 50)}...` : text;
    ...
    const DuyuruTableKontrol = function () {
        document
            .querySelector("#duyuruDataTable")
        ...
        duyuruDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => duyuruDataTable.row(row.node).data().sira_no)
                .sort((a, b) => a - b);

            const duyurular = details.map((row, index) => {
                return {
                    id: duyuruDataTable.row(row.node).data().id,
            ...
            if (duyurular.length > 0) {
                ...
                axios.post(`${panelUrl}/duyuru/siralamaduzenle`, { duyurular: duyurular })
    ...
    const handleDelete = (e) => {
        ...
        const data = duyuruDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Duyuru Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/duyuru/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
