...
const RPKatalogKategoriIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let katalogKategoriDataTable;

    const KatalogKategoriTable = function () {
        katalogKategoriDataTable = AjaxDataTables({
            id: "katalogKategoriDataTable",
            ajax: {
                url: `${panelUrl}/katalogkategori/tableliste`,
                data: function (data) {
            ...
            buttons: {
                print: {
                    dosyaAdi: "katalogKategori-liste",
                    baslik: "Katalog Kategori Liste",
                    tabloSutun: [0, 1, 2],
                },
    ...
    const KatalogKategoriTableKontrol = function () {
        document
            .querySelector("#katalogKategoriDataTable")
        ...
        katalogKategoriDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => katalogKategoriDataTable.row(row.node).data().sira_no)
                .sort((a, b) => a - b);

            const kategoriler = details.map((row, index) => {
                return {
                    id: katalogKategoriDataTable.row(row.node).data().id,
            ...
            if (kategoriler.length > 0) {
                ...
                axios.post(`${panelUrl}/katalogkategori/siralamaduzenle`, { kategoriler: kategoriler })
    ...
    const handleDelete = (e) => {
        ...
        const data = katalogKategoriDataTable.row($(parent)).data();

        // Kategoriye bağlı ürünleri kontrol et
        if (data.toplam_katalog == 0) {
            Swall({
                icon: "warning",
                icerik: "Katalog Kategori Silinicek Onaylıyormusunuz!",
                iptalButonDurum: true,
            }).then((result) => {
                if (result.value) {
                    axios
                        .post(`${panelUrl}/katalogkategori/sil/${data.id}`)
                        .then((response) => {
                            Swall({
                                icon: "success",
            ...
            axios
                .get(`${panelUrl}/katalogkategori/liste`, { params: { katalogKategori: data.id } })
                .then((response) => {
                    let options = ``;
            ...
            document.querySelector("#transferOnayBtn").addEventListener('click', function () {
                ...
                Swall({
                    icon: "question",
                    icerik: "Kataloglerinizi bu kategoriye aktarmak istediğinize emin misiniz?",
                    iptalButonDurum: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios
                            .post(`${panelUrl}/katalogkategori/transfer`, {
    ...
    return {
        init: function () {
            KatalogKategoriTable();
            KatalogKategoriTableKontrol();
        },
    };
})();

document.addEventListener("DOMContentLoaded", (event) => {
    RPKatalogKategoriIndex.init();
});
