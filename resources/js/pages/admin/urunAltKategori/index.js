...
const RPUrunAltKategoriIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let urunAltKategoriDataTable;

    const UrunAltKategoriTable = function () {
        urunAltKategoriDataTable = AjaxDataTables({
            id: "urunAltKategoriDataTable",
            ajax: {
                url: `${panelUrl}/urunaltkategori/tableliste`,
                data: function (data) {
            ...
            buttons: {
                print: {
                    dosyaAdi: "urunAltKategori-liste",
                    baslik: "Ürün Alt Kategori Liste",
                    tabloSutun: [1, 2, 3],
                },
        ...
        urunAltKategoriDataTable.on('row-reorder', function (e, details, changes) {
            const sortedSiraNo = details
                .map(row => urunAltKategoriDataTable.row(row.node).data().sira_no) // sira_no değerlerini al
                .sort((a, b) => a - b); // Küçükten büyüğe sırala

            // 2. details.map döngüsünde sıralanmış sira_no değerlerini sırayla ata
            const altKategoriler = details.map((row, index) => {
                return {
                    id: urunAltKategoriDataTable.row(row.node).data().id, // ID'yi al
            ...
            if (altKategoriler.length > 0) {
                ...
                axios.post(`${panelUrl}/urunaltkategori/siralamaduzenle`, { altKategoriler: altKategoriler })
        ...
        /* multiCheck(urunAltKategoriDataTable); */
    };
    const UrunAltKategoriTableKontrol = function () {
        document
            .querySelector("#urunAltKategoriDataTable")
    ...
    const handleDelete = (e) => {
        ...
        const data = urunAltKategoriDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Ürün Alt Kategori Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/urunaltkategori/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
    ...
    return {
        init: function () {
            UrunAltKategoriTable();
            UrunAltKategoriTableKontrol();
        },
    };
})();

document.addEventListener("DOMContentLoaded", (event) => {
    RPUrunAltKategoriIndex.init();
});
