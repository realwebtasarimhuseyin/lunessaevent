...
var RPKullaniciIndex = (function () {
    const { AjaxDataTables, ZamanFormati, Swall } = useReal();
    let kullaniciDataTable;

    let KullaniciTable = function () {
        kullaniciDataTable = AjaxDataTables({
            id: "kullaniciDataTable",
            ajax: {
                url: `${panelUrl}/kullanici/liste`,
                data: function (data) {
            ...
            buttons: {
                print: {
                    dosyaAdi: "kullanici-liste",
                    baslik: "Kullanici Liste",
        ...
        multiCheck(kullaniciDataTable);
    };

    const KullaniciTableKontrol = function () {
        document
            .querySelector("#kullaniciDataTable")
    ...
    const handleDelete = (e) => {
        ...
        const data = kullaniciDataTable.row($(parent)).data();

        Swall({
            icon: "warning",
            icerik: "Kullanıcı Silinicek Onaylıyormusunuz!",
            iptalButonDurum: true,
        }).then((result) => {
            if (result.value) {
                axios
                    .post(`${panelUrl}/kullanici/sil/${data.id}`)
                    .then((response) => {
                        Swall({
                            icon: "success",
    ...
    return {
        init: function () {
            KullaniciTable();
            KullaniciTableKontrol();
        },
    };
})();

document.addEventListener("DOMContentLoaded", (event) => {
    RPKullaniciIndex.init();
});
